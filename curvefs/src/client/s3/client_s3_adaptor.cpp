...
CURVEFS_ERROR S3ClientAdaptorImpl::FlushDataCache(const UperFlushRequest& req, uint64_t* writeOffset) {
    ...
    LOG(ERROR) << "whs FlushDataCache 11!" <<  s3Tasks.size()
               << ", " << kvCacheTasks.size();
...
CURVEFS_ERROR S3ClientAdaptorImpl::PrepareFlushTasks(const UperFlushRequest& req,
  std::vector<std::shared_ptr<PutObjectAsyncContext>> *s3Tasks,
  std::vector<std::shared_ptr<SetKVCacheTask>> *kvCacheTasks,
...
void S3ClientAdaptorImpl::FlushTaskExecute(
    ...
    VLOG(0) << "whs FlushTaskExecute start 00";
    // callback
...
/*
                if (s3Metric_.get() != nullptr) {
                    CollectMetrics(s3Metric_->adaptorWriteS3,
...
VLOG(0) << "whs FlushTaskExecute start 01";
...
VLOG(0) << "whs FlushTaskExecute start 02";
    SetKVCacheDone kvdone = [&](const std::shared_ptr<SetKVCacheTask> &task) {
        kvTaskEnvent.Signal();
VLOG(0) << "whs FlushTaskExecute start 03";
        return;
    };
VLOG(0) << "whs FlushTaskExecute start 04";
    // s3task execute
    if (s3PendingTaskCal.load()) {
        VLOG(0) << "whs FlushTaskExecute start 04.1";
        std::for_each(
...
VLOG(0) << "whs FlushTaskExecute start 05";
    // kvtask execute
    ...
    VLOG(0) << "whs FlushTaskExecute start 06";
...
CURVEFS_ERROR S3ClientAdaptorImpl::ReadFromLowlevel(UperReadRequest request) {
    ...
    do {
        ...
        int ret = ReadKVRequest(uperRequests, readBuf, inodeWrapper->GetLength());
        if (ret >= 0) {
            // read ok
            break;
        } else if (ret == -2) {
...
int S3ClientAdaptorImpl::HandleReadS3NotExist(
    ...
    const std::shared_ptr<InodeWrapper> &inodeWrapper) {
    uint32_t maxIntervalMs = GetMaxReadRetryIntervalMs();  // hardcode, fixme
...
/*
    if (3Metric_.get()) {
        CollectMetrics( &s3ClientAdaptor_->s3Metric_->adaptorReadS3, len, start);
    }
*/
    return true;
}
...
CURVEFS_ERROR S3ClientAdaptorImpl::Truncate(InodeWrapper *inodeWrapper, uint64_t size) {
